function [var,loni,lati] = get_SeaWiFS_data(SeaWiFS_file,variable,month,lon,lat,BRYtime,days);
...
%%%% FIND the file to be read %%%%%%
  infile = SeaWiFS_file ;
  disp(['get_SeaWiFS : reading file : ' infile])  
  
%%%% read lon/lat %%%%%%
  loni = ncread(infile,'longitude')'; 
  disp('WARNING : special fix on longitude')
  loni = [loni(2161:end) loni(1:2160)+360];
  lati = ncread(infile,'latitude')';
  lati = lati(end:-1:1) ;
...
%   var = squeeze(var(2,:,:)) ;
  ...
  var = [var(end:-1:1,2161:end) var(end:-1:1,1:2160)];
  var = var(j0:j1,i0:i1);
  var(var<0.005)=0.005 ;  % avoid "dead water"
