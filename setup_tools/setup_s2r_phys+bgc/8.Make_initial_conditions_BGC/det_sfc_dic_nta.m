function [srf_NTA_vol,srf_dic_vol]=det_sfc_dic_nta(ininame,grdname,pars)
...
%
srf_po4 = ncread(ininame,'PO4') ; srf_po4 = squeeze(srf_po4(:,:,end))' ;
srf_sil = ncread(ininame,'SiO3'); srf_sil = squeeze(srf_sil(:,:,end))' ;

%
% T E M P   a n d   S A L T   d a t a
%
srf_temp = ncread(ininame,'temp'); srf_temp = squeeze(srf_temp(:,:,end))' ;
srf_salt = ncread(ininame,'salt'); srf_salt = squeeze(srf_salt(:,:,end))' ;
...
srf_density =  sw_dens0(srf_salt,srf_temp);
    ...
    basin_indx = ncread(ininame,'basindx')' ;
