...
grdname  = '/glade/derecho/scratch/bachman/ROMS_tools/MARBL_REALISTIC/1.Make_grid/roms_grd.nc';
root_name= '/glade/derecho/scratch/bachman/ROMS_tools/MARBL_REALISTIC/1.Make_grid/roms';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   datname = '/glade/derecho/scratch/bachman/ROMS_tools/DATASETS/BGC/spco2_aco2_clim_ETH_SOM-FFN_CDIAC_G05_corr.nc';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   datname = '/glade/derecho/scratch/bachman/ROMS_tools/DATASETS/BGC/solFe_scenario4_current_gx1v6.nc';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   datname = '/glade/derecho/scratch/bachman/ROMS_tools/DATASETS/BGC/dst79gnx_gx1v6_090416.nc';
   factor = 1 ;
   lon_frc = ncread(datname,'TLONG');% lon_frc(lon_frc>0) = lon_frc(lon_frc>0)-360 ;
   lat_frc = ncread(datname,'TLAT');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   disp(' ')
   disp(' Read in the NOx file ')

   datname = '/glade/derecho/scratch/bachman/ROMS_tools/DATASETS/BGC/ndep_ocn_1850_w_nhx_emis_MOM_tx0.66v1_c230821.nc';
   factor = 1 ;
   staticname = '/glade/work/mlevy/cesm_inputdata/tx0.66v1.static.nc';
   lon_frc = ncread(staticname,'geolon');
   lon_frc(lon_frc < 360) = lon_frc(lon_frc < 360) + 360;
   lat_frc = ncread(staticname,'geolat');

   %lon_frc = ncread(datname,'TLONG');% lon_frc(lon_frc>0) = lon_frc(lon_frc>0)-360 ;
   %lat_frc = ncread(datname,'TLAT');

   create_frc_BGC(grdname,frcname,coarse_frc,'NOx');
   NOx_time = [0.5:11.5]*365.25/12; % Months of equal length
   ncwrite(frcname,'NOx_time',NOx_time);
      % Loop over data files
   for i = 1:12
     disp([' Processing month : ' num2str(i)])
     data = ncread(datname,'NDEP_NOy_month',[1 1 i],[inf inf 1]);
     data(data<0) = NaN ;
     NOx = griddata(double(lon_frc),double(lat_frc),double(data),lon,lat);
     NOx = inpaint_nans(NOx,2);
     NOx = NOx * factor ;
     ncwrite(frcname,'NOx',NOx,[1 1 i])
   end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   disp(' ')
   disp(' Read in the NHy file ')
   ...
   create_frc_BGC(grdname,frcname,coarse_frc,'NHy');
   NHy_time = [0.5:11.5]*365.25/12; % Months of equal length
   ncwrite(frcname,'NHy_time',NHy_time);
      % Loop over data files
   for i = 1:12
     disp([' Processing month : ' num2str(i)])
     data = ncread(datname,'NDEP_NHx_month',[1 1 i],[inf inf 1]);
     data(data<0) = NaN ;
     NHy = griddata(double(lon_frc),double(lat_frc),double(data),lon,lat);
     NHy = inpaint_nans(NHy,2);
     NHy = NHy * factor ;
     ncwrite(frcname,'NHy',NHy,[1 1 i])
   end
